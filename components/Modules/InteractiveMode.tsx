...
// --- CONFIGURATION ---
// Medical Book Toggle: loads Emergency Medical Guide content
const DEFAULT_USE_MEDICAL_BOOK = false;
// Cache Toggle: uses Google Context Cache (only works when Medical Book is ON)
const DEFAULT_USE_CACHE = false;

// Model name from environment variable
const GEMINI_MODEL = process.env.GEMINI_MODEL || 'gemini-3-flash-preview';

// Context Cache - Default name (from create_cache.js)
const DEFAULT_CACHE_NAME = "cachedContents/3lnctfmso6h38556kau94a3qzqbqhn1ur2jdbt1w";

// Medical book file path (served from public/ folder)
const MEDICAL_BOOK_PATH = '/Emergency_Medical_Guide.txt';

// System instruction for cached medical mode
const CACHE_SYSTEM_INSTRUCTION = `You are ResQ-AI, an expert emergency medical assistant with comprehensive medical knowledge from the uploaded Emergency Medical Guide.
...
// --- CRITICAL SEVERITY DETECTION (Client-Side Backup) ---
// Detects critical emergencies from BOTH user input AND AI responses
// This ensures critical mode activates even if the AI forgets ||CRITICAL_STATE||
const CRITICAL_PATTERNS = {
  // User input keywords (what the user might say)
  userKeywords: [
    'bleeding', 'blood', 'hemorrhage', 'haemorrhage',
    'not breathing', 'cant breathe', 'can\'t breathe', 'cannot breathe', 'no pulse', 'stopped breathing',
    'unconscious', 'unresponsive', 'passed out', 'fainted', 'collapsed',
    'choking', 'choke', 'heimlich', 'airway blocked',
    'heart attack', 'cardiac arrest', 'chest pain', 'heart stopped',
    'cpr', 'compressions',
    'stroke', 'face drooping', 'slurred speech',
    'seizure', 'convulsion', 'fitting',
    'anaphylaxis', 'allergic reaction', 'epipen', 'throat swelling', 'swollen throat',
    'severe burn', 'on fire', 'caught fire',
    'drowning', 'drowned', 'underwater',
    'overdose', 'poisoning', 'poisoned', 'swallowed poison',
    'stabbed', 'stabbing', 'gunshot', 'shot',
    'dying', 'going to die', 'not moving', 'no heartbeat',
    'head injury', 'skull fracture', 'head trauma',
    'electric shock', 'electrocuted',
    'snake bite', 'bitten by snake',
    'help', 'emergency', 'urgent', '911'
  ],
  // AI response keywords (what the AI might say indicating severity)
  responseKeywords: [
    'call 911', 'call emergency', 'call ambulance', 'call paramedics',
    'life-threatening', 'life threatening', 'immediately dangerous',
    'extremely dangerous', 'seek emergency', 'emergency room immediately',
    'go to the nearest emergency', 'rush to hospital',
    'cardiac arrest', 'not breathing', 'no pulse',
    'apply direct pressure', 'severe laceration', 'significant bleeding',
    'heavy bleeding', 'arterial bleeding', 'tourniquet',
    'perform cpr', 'start cpr', 'begin compressions',
    'anaphylactic shock', 'severe allergic',
    'loss of consciousness', 'unresponsive',
    'stroke symptoms', 'brain hemorrhage',
...
// Check if text contains critical patterns
function detectCriticalSeverity(text: string, type: 'user' | 'response'): boolean {
  const lowerText = text.toLowerCase();
  const keywords = type === 'user' ? CRITICAL_PATTERNS.userKeywords : CRITICAL_PATTERNS.responseKeywords;
...
export const InteractiveMode: React.FC<InteractiveModeProps> = ({ onBack, isCritical, setCritical, onEmergency }) => {
  ...
  // Medical Book Toggle: true = use medical book content
  const [useMedicalBook, setUseMedicalBook] = useState(DEFAULT_USE_MEDICAL_BOOK);
  // Cache Toggle: true = use Google Context Cache (only when book is ON)
  const [useCache, setUseCache] = useState(DEFAULT_USE_CACHE);

  // Cache loading state
  const [isCacheLoading, setIsCacheLoading] = useState(false);
  const [cacheLoadingStep, setCacheLoadingStep] = useState('');
  const cacheNameRef = useRef<string>(DEFAULT_CACHE_NAME);
...
If critical (heavy bleeding, unconscious, not breathing), start with ||CRITICAL_STATE||`;
      } else {
...
6. If you see anything life-threatening (heavy bleeding, unconscious, not breathing, severe burns, deep wounds), ALWAYS start your response with ||CRITICAL_STATE||
Example responses: "Good. Keep that pressure." / "Move it left a bit." / "Looking better, keep going."` }]
      ...
      if (isMountedRef.current) {
        ...
        // Check for critical response - AI trigger + client-side backup
        const isCriticalResponse = responseText.includes("||CRITICAL_STATE||") || responseText.includes("CRITICAL_ALERT") || detectCriticalSeverity(responseText, 'response');
...
6. If you see anything life-threatening (heavy bleeding, unconscious, not breathing, severe burns, deep wounds), ALWAYS start your response with ||CRITICAL_STATE||` }]
      ...
      if (isMountedRef.current) {
        ...
        if (responseText.includes("||CRITICAL_STATE||") || responseText.includes("CRITICAL_ALERT") || detectCriticalSeverity(responseText, 'response')) {
          setCritical(true);
          if (!isMicActive) setIsMicActive(true);
        }
      }
...
If critical (heavy bleeding, severe injury, unconscious), start with ||CRITICAL_STATE||`;
      } else {
        // Follow-up check
...
If situation worsened or critical, start with ||CRITICAL_STATE||`;
      }
...
6. If you see anything life-threatening (heavy bleeding, unconscious, not breathing, severe burns, deep wounds), ALWAYS start your response with ||CRITICAL_STATE||` }]
  ...
  // --- Upload Medical Book to Google Files API ---
  const uploadMedicalBook = async (ai: any): Promise<string> => {
    // Step 1: Fetch the book from public folder
    setCacheLoadingStep('Downloading medical library...');
    const response = await fetch(MEDICAL_BOOK_PATH);
    if (!response.ok) throw new Error('Failed to fetch medical book file');
    const blob = await response.blob();

    // Step 2: Upload to Google Files API
    setCacheLoadingStep('Uploading medical library to AI...');
    const uploadedFile = await ai.files.upload({
      file: blob,
      config: {
        displayName: 'Emergency_Medical_Guide.txt',
        mimeType: 'text/plain'
      }
    });

    // Step 3: Wait for file processing to complete
    setCacheLoadingStep('Processing medical library...');
    ...
    if (file.state === 'FAILED') {
      throw new Error('Medical book file processing failed');
    }

    console.log("[DEBUG] Medical book uploaded, URI:", file.uri);
    return file.uri;
  };
  ...
  useEffect(() => {
    const initChat = async () => {
      ...
      try {
        ...
        // Create Chat Session based on toggle states
        console.log(`[DEBUG] Creating Chat Session (Book: ${useMedicalBook}, Cache: ${useCache})...`);

        // System instruction for general LLM mode (no book)
...
Remember: This is a conversation. One step, wait for response, then next step.`;
        ...
        if (useMedicalBook && useCache) {
          // ===========================================
          // MODE 1: MEDICAL BOOK + CACHE (Full pipeline)
          // ===========================================
          setIsCacheLoading(true);
          setCacheLoadingStep('Checking cache status...');

          let activeCacheName: string | null = null;

          try {
            console.log("[DEBUG] Checking existing cache:", cacheNameRef.current);
            setCacheLoadingStep('Verifying medical library cache...');
            const existingCache = await ai.caches.get({ name: cacheNameRef.current });

            const expireTime = new Date(existingCache.expireTime || 0);
            if (expireTime > new Date()) {
              console.log("[DEBUG] Cache is ALIVE. Expires:", expireTime);
              activeCacheName = cacheNameRef.current;
              setCacheLoadingStep('Cache valid! Loading...');
            } else {
              console.log("[DEBUG] Cache EXPIRED at:", expireTime);
              activeCacheName = null;
            }
          } catch (checkError: any) {
            console.log("[DEBUG] Cache check failed:", checkError.message);
            activeCacheName = null;
          }

          // If no valid cache, search or create new
          if (!activeCacheName) {
            try {
              setCacheLoadingStep('Searching for existing caches...');
              try {
                const cacheList = await ai.caches.list();
                for await (const cache of cacheList) {
                  if (cache.displayName === 'ResQ-AI Medical Guide Cache') {
                    const exp = new Date(cache.expireTime || 0);
                    if (exp > new Date()) {
                      console.log("[DEBUG] Found existing valid cache:", cache.name);
                      activeCacheName = cache.name || null;
                      cacheNameRef.current = cache.name || DEFAULT_CACHE_NAME;
                      setCacheLoadingStep('Found existing cache!');
                      break;
                    }
                  }
                }
              } catch (listError) {
                console.log("[DEBUG] Cache list failed, will create new");
              }

              // Still no cache? Upload book + create new cache
              if (!activeCacheName) {
                const fileUri = await uploadMedicalBook(ai);

                setCacheLoadingStep('Creating AI knowledge cache...');
                ...
                if (activeCacheName) {
                  cacheNameRef.current = activeCacheName;
                  console.log("[DEBUG] New cache created:", activeCacheName);
                  setCacheLoadingStep('Cache created successfully!');
                }
              }
            } catch (createError: any) {
              console.error("[DEBUG] Cache creation failed:", createError.message);
              setCacheLoadingStep('Cache creation failed. Falling back to inline book mode...');
            }
          }
          ...
          if (activeCacheName) {
            ...
            if (isMountedRef.current) {
              ...
              setMessages([{
                id: '1',
                role: 'assistant',
                content: 'Medical Library Loaded (Cached). I am ready to assist.',
                timestamp: new Date(),
                type: 'text'
              }]);
            }
          } else {
            // Cache failed → fallback to inline book mode (Mode 2)
            console.log("[DEBUG] Cache failed, falling back to inline book mode...");
            try {
              setCacheLoadingStep('Loading book content directly...');
              setIsCacheLoading(true);
              const bookResponse = await fetch(MEDICAL_BOOK_PATH);
              const bookText = await bookResponse.text();
              setIsCacheLoading(false);
              setCacheLoadingStep('');

              const bookInstruction = CACHE_SYSTEM_INSTRUCTION + `\n\n--- EMERGENCY MEDICAL GUIDE ---\n${bookText}\n--- END OF GUIDE ---`;
              chatSessionRef.current = ai.chats.create({
                model: GEMINI_MODEL,
                config: { systemInstruction: bookInstruction },
                history: []
              });
              ...
              if (isMountedRef.current) {
                ...
                setMessages([{
                  id: '1',
                  role: 'assistant',
                  content: 'Cache unavailable. Medical Book loaded inline. I am ready to assist.',
                  timestamp: new Date(),
                  type: 'text'
                }]);
              }
              ...
              chatSessionRef.current = ai.chats.create({
                model: GEMINI_MODEL,
                config: { systemInstruction: GENERAL_SYSTEM_INSTRUCTION },
                history: []
              });
              if (isMountedRef.current) {
                setIsMicActive(true);
                setUseMedicalBook(false);
                setUseCache(false);
                setMessages([{
                  id: '1',
                  role: 'assistant',
                  content: 'Failed to load medical resources. Running in General AI mode.',
                  timestamp: new Date(),
                  type: 'text'
                }]);
              }
        ...
        } else if (useMedicalBook && !useCache) {
          // ===========================================
          // MODE 2: MEDICAL BOOK INLINE (No cache)
          // ===========================================
          try {
            setIsCacheLoading(true);
            setCacheLoadingStep('Loading medical book content...');

            const bookResponse = await fetch(MEDICAL_BOOK_PATH);
            if (!bookResponse.ok) throw new Error('Failed to fetch medical book');
            const bookText = await bookResponse.text();

            setIsCacheLoading(false);
            setCacheLoadingStep('');

            // Append book content directly into the system instruction
            const bookInstruction = CACHE_SYSTEM_INSTRUCTION + `\n\n--- EMERGENCY MEDICAL GUIDE ---\n${bookText}\n--- END OF GUIDE ---`;
            ...
            if (isMountedRef.current) {
              ...
              setMessages([{
                id: '1',
                role: 'assistant',
                content: 'Medical Book Loaded (Inline). I am ready to assist.',
                timestamp: new Date(),
                type: 'text'
              }]);
            }
          } catch (bookError: any) {
            console.error("[DEBUG] Book loading failed:", bookError.message);
            setIsCacheLoading(false);
            setCacheLoadingStep('');
            ...
            if (isMountedRef.current) {
              ...
              setMessages([{
                id: '1',
                role: 'assistant',
                content: 'Book loading failed. Running in General AI mode.',
                timestamp: new Date(),
                type: 'text'
              }]);
            }
          ...
          // ===========================================
          // MODE 3: GENERAL LLM (No book, no cache)
          // ===========================================
          chatSessionRef.current = ai.chats.create({
            model: GEMINI_MODEL,
            config: { systemInstruction: GENERAL_SYSTEM_INSTRUCTION },
  ...
  }, [useMedicalBook, useCache]);

  // Reinitialize chat when toggles change
  const reinitChat = (message: string) => {
  ...
  // Toggle Medical Book ON/OFF
  const handleToggleBook = () => {
    const newMode = !useMedicalBook;
    setUseMedicalBook(newMode);
    if (!newMode) setUseCache(false); // Turn off cache when book is off
    reinitChat(newMode ? 'Loading Medical Book...' : 'Switching to General AI Mode...');
  };

  // Toggle Cache ON/OFF (only works when book is ON)
  const handleToggleCache = () => {
    const newMode = !useCache;
    setUseCache(newMode);
    reinitChat(newMode ? 'Switching to Cached Mode...' : 'Switching to Inline Book Mode...');
  };
  ...
  const handleSendMessage = async (overrideText?: string) => {
    ...
    // Client-side critical detection on user input
    if (detectCriticalSeverity(textToSend, 'user')) {
      setCritical(true);
      if (!isMicActive) setIsMicActive(true);
    }
    ...
    try {
      ...
      // Check 1: AI explicitly sent the trigger tag
      // Check 2: Client-side backup - detect critical keywords in AI response
      const hasAITrigger = fullResponseText.includes("||CRITICAL_STATE||") || fullResponseText.includes("CRITICAL_ALERT");
      const hasResponseCritical = detectCriticalSeverity(fullResponseText, 'response');

      if (hasAITrigger || hasResponseCritical) {
          setCritical(true);
          if (!isMicActive) setIsMicActive(true);
      }
  ...
  return (
    <div className={`flex flex-col h-screen relative overflow-hidden transition-all duration-500 ${
      ...
      {isCacheLoading && (
        <div className="absolute inset-0 z-[100] bg-slate-950/90 backdrop-blur-md flex flex-col items-center justify-center animate-fade-in">
          <div className="flex flex-col items-center gap-4 max-w-sm text-center px-6">
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 w-16 h-16 rounded-full border-2 border-blue-400/30 border-t-blue-400 animate-spin" />
            </div>

            {/* Loading Text */}
            <div>
              <h3 className="text-white font-bold text-lg">Loading Medical Library</h3>
              <p className="text-blue-300 text-sm mt-1 animate-pulse">{cacheLoadingStep}</p>
            </div>
      ...
      <div className="absolute top-0 left-0 right-0 z-50 p-4 flex justify-between items-start pointer-events-none">
          ...
          {/* Toggle Buttons + End Emergency Button */}
          <div className="pointer-events-auto flex items-center gap-2">
             {/* Toggles - Only visible when not in critical mode */}
             {!isCritical && (
               <div className="flex items-center gap-2">
                 {/* Medical Book Toggle */}
                 <button
                   onClick={handleToggleBook}
                   disabled={isThinking || isCacheLoading}
                   className={`flex items-center gap-2 px-3 py-2 rounded-xl backdrop-blur-md shadow-lg border transition-all text-xs font-bold ${
                     useMedicalBook
                       ? 'bg-emerald-500/15 border-emerald-400/30 text-emerald-400'
                       : 'bg-slate-500/15 border-slate-400/30 text-slate-400'
                   } ${(isThinking || isCacheLoading) ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105 active:scale-95 cursor-pointer'}`}
                 >
                   <Icons.Learning size={16} />
                   <span>Medical Book</span>
                   <div className={`w-9 h-5 rounded-full relative transition-colors ${useMedicalBook ? 'bg-emerald-500' : 'bg-slate-500'}`}>
                     <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-md transition-transform ${useMedicalBook ? 'translate-x-4.5' : 'translate-x-0.5'}`} />
                   </div>
                 </button>
                 ...
                 <button
                   ...
                   disabled={isThinking || isCacheLoading || !useMedicalBook}
                   className={`flex items-center gap-2 px-3 py-2 rounded-xl backdrop-blur-md shadow-lg border transition-all text-xs font-bold ${
                     useCache
                   ...
                   } ${(isThinking || isCacheLoading || !useMedicalBook) ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105 active:scale-95 cursor-pointer'}`}
                 >
                   <Icons.AI size={16} />
                   <span>Cache</span>
                   <div className={`w-9 h-5 rounded-full relative transition-colors ${useCache ? 'bg-blue-500' : 'bg-slate-500'}`}>
                     <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-md transition-transform ${useCache ? 'translate-x-4.5' : 'translate-x-0.5'}`} />
                   </div>
                 </button>
               </div>
             )}
      ...
      {/* Manual Critical Trigger (Only if not critical) - positioned below toggle */}
      {!isCritical && (
        <button onClick={() => setCritical(true)} className="absolute top-16 right-4 bg-red-600 hover:bg-red-700 transition-colors text-white px-4 py-1.5 text-xs font-bold tracking-widest text-center animate-pulse z-30 shadow-md rounded-lg">
          TAP FOR EMERGENCY
        </button>
      )}
      ...
      <div className={`shrink-0 p-3 sm:p-4 border-t z-30 transition-colors duration-300 safe-area-bottom ${
          ...
          <div className={`text-center mt-1.5 sm:mt-2 text-[11px] sm:text-xs flex items-center justify-center gap-2 ${isCritical ? 'text-slate-400' : 'text-slate-500'}`}>
             ...
             <div className="hidden sm:flex items-center gap-1 px-1.5 py-0.5 rounded bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-200 dark:border-blue-800/30 ml-2">
                <Icons.AI size={10} className="text-blue-500" /> <span className="text-[9px] font-bold text-blue-600 dark:text-blue-400">{useMedicalBook && useCache ? `${GEMINI_MODEL} (Cached)` : useMedicalBook ? `${GEMINI_MODEL} (Book)` : `${GEMINI_MODEL} (General)`}</span>
             </div>
          </div>
        </div>
    </div>
